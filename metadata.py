...
def _normalize_multiline_quote(text: str) -> str:
    ...
    for line in lines + [""]:
        if line.startswith(">"):
            quote_block.append(line[1:].strip())
        else:
            if quote_block:
...
class Track_parsed:
    ...
    _TYPE_MAP = {
        "pre": "Pre", "bold": "Bold", "mono": "Mono", "italic": "Italic",
        "underline": "Underline", "strike": "Strike", "spoiler": "Spoiler",
        "quote": "Quote", "quote_md": "Quote", "link": "Link", "mention": "MentionText",
    }
    ...
    def transcribe(self, src: str, mode: str = "MARKDOWN") -> dict:
        ...
        for m in matches:
            ...
            if gname == "link":
                ...
                if link_href and link_href.startswith("@"):
                    link_href = f"Rubika://l.rubika.ir/{link_href[1:]}"
                if link_href and link_href.startswith("09"):
                    link_href = f"tel:{link_href}"
                if link_href and link_href.startswith("u0"):
                    gname = "mention"
            else:
        ...
        result = {"text": normalized_text.strip()}
        if payload_parts:
            result["metadata"] = {"meta_data_parts": payload_parts}
        return result

    def parse(self, text: str, parse_mode: str = "MARKDOWN") -> dict:
        return self.transcribe(text, mode=parse_mode)

GlyphWeaver = Track_parsed